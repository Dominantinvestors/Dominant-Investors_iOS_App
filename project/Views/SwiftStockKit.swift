            ...
            Alamofire.request(stockURL).responseJSON { response in
                ...
                if let resultJSON = response.result.value as? [String : AnyObject]  {
                    ...
                    if let stockDataArray = resultJSON["results"] as? [[String : AnyObject]] {
                        ...
                        newDataFields.append(["Net Change" : String(describing: stockData["netChange"] as! CGFloat) ])
                        newDataFields.append(["Name" : stockData["name"] as? String ?? "N/A"])
                        newDataFields.append(["Unit Code" : stockData["unitCode"] as? String ?? "N/A"])
                        newDataFields.append(["Open price" : String(describing: stockData["open"] as! CGFloat) ])
                        newDataFields.append(["Last Price" : String(describing: stockData["lastPrice"] as! CGFloat) ])
                        newDataFields.append(["Volume" : String(describing: stockData["volume"] as! CGFloat) ])
                        newDataFields.append(["24h Low" : String(describing: stockData["low"] as! CGFloat) ])
                        newDataFields.append(["24h High" : String(describing: stockData["high"] as! CGFloat) ])
                        newDataFields.append(["Last close" : String(describing: stockData["close"] as! CGFloat) ])
